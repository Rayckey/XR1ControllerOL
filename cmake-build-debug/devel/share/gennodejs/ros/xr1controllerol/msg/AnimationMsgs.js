// Auto-generated. Do not edit!

// (in-package xr1controllerol.msg)


"use strict";

const _serializer = _ros_msg_utils.Serialize;
const _arraySerializer = _serializer.Array;
const _deserializer = _ros_msg_utils.Deserialize;
const _arrayDeserializer = _deserializer.Array;
const _finder = _ros_msg_utils.Find;
const _getByteLength = _ros_msg_utils.getByteLength;

//-----------------------------------------------------------

class AnimationMsgs {
  constructor(initObj={}) {
    if (initObj === null) {
      // initObj === null is a special case for deserialization where we don't initialize fields
      this.AnimationType = null;
      this.AnimationID = null;
    }
    else {
      if (initObj.hasOwnProperty('AnimationType')) {
        this.AnimationType = initObj.AnimationType
      }
      else {
        this.AnimationType = 0;
      }
      if (initObj.hasOwnProperty('AnimationID')) {
        this.AnimationID = initObj.AnimationID
      }
      else {
        this.AnimationID = 0;
      }
    }
  }

  static serialize(obj, buffer, bufferOffset) {
    // Serializes a message object of type AnimationMsgs
    // Serialize message field [AnimationType]
    bufferOffset = _serializer.int32(obj.AnimationType, buffer, bufferOffset);
    // Serialize message field [AnimationID]
    bufferOffset = _serializer.int32(obj.AnimationID, buffer, bufferOffset);
    return bufferOffset;
  }

  static deserialize(buffer, bufferOffset=[0]) {
    //deserializes a message object of type AnimationMsgs
    let len;
    let data = new AnimationMsgs(null);
    // Deserialize message field [AnimationType]
    data.AnimationType = _deserializer.int32(buffer, bufferOffset);
    // Deserialize message field [AnimationID]
    data.AnimationID = _deserializer.int32(buffer, bufferOffset);
    return data;
  }

  static getMessageSize(object) {
    return 8;
  }

  static datatype() {
    // Returns string type for a message object
    return 'xr1controllerol/AnimationMsgs';
  }

  static md5sum() {
    //Returns md5sum for a message object
    return '9e4a55e887ede9012b0ab385278da204';
  }

  static messageDefinition() {
    // Returns full string definition for message
    return `
    int32 AnimationType
    int32 AnimationID
    `;
  }

  static Resolve(msg) {
    // deep-construct a valid message object instance of whatever was passed in
    if (typeof msg !== 'object' || msg === null) {
      msg = {};
    }
    const resolved = new AnimationMsgs(null);
    if (msg.AnimationType !== undefined) {
      resolved.AnimationType = msg.AnimationType;
    }
    else {
      resolved.AnimationType = 0
    }

    if (msg.AnimationID !== undefined) {
      resolved.AnimationID = msg.AnimationID;
    }
    else {
      resolved.AnimationID = 0
    }

    return resolved;
    }
};

module.exports = AnimationMsgs;
