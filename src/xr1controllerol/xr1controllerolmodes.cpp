...
void XR1ControllerOL::setControlMode(uint8_t control_group, uint8_t option) {
    ...
    if (allActuatorHasLaunched() && RecognizeFinished){
        ...
        if (control_modes[control_group] == option) {
            ...
            if (temp_mode == XR1::VelocityMode && control_group == XR1::OmniWheels){ // only for omni wheels , set profile velociy mode
            ...
            else if (temp_mode == XR1::MoCapMode && control_group == XR1::OmniWheels){ // only for omni wheels , set profile velociy mode
