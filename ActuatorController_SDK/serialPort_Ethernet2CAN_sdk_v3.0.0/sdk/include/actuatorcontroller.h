...
//    void requestQuaternion(const std::string & target);
    ...
    double batteryCellVoltage(uint8_t index);
    bool batteryCellIsBalance(uint8_t index);
    uint16_t batteryLoopCount();
    double batteryDumpEnergy();
    double batteryTotalEnergy();
    bool isChargeOpen();
    bool isDischargeOpen();
    uint8_t batteryTempSensorCount();
