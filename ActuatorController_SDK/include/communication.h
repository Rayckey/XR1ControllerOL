...
//public slots:
    ...
    void setSoftwareVersion(uint32_t version,uint32_t nUnitId);
    uint32_t readSoftwareVersion(uint32_t nUnitId)const;
    void setHardwareVersion(uint32_t version,uint32_t nUnitId);
    uint32_t readHardwareVersion(uint32_t nUnitId)const;
protected:
